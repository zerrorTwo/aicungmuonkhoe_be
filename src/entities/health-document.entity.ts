...
export class HealthDocument {
    ...
    @Column({ name: 'HEALTH_STATUS', type: 'varchar', length: 100, nullable: true })
    ...
    @Column({ name: 'DATE_WORKDAY', type: 'varchar', length: 100, nullable: true })
    DATE_WORKDAY: string;
    ...
    @Column({ name: 'EXERCISE_FREQUENCY', type: 'varchar', length: 100, nullable: true })
    EXERCISE_FREQUENCY: string;

    @Column({ name: 'PROVINCE', type: 'varchar', length: 100, nullable: true })
    PROVINCE: string;

    @ManyToOne(() => Gender, (gender: Gender) => gender.HEALTH_DOCUMENTS, { nullable: true })
    @JoinColumn({ name: 'GENDER_ID' })
    GENDER: Gender;

    @ManyToOne(
        () => ExerciseIntensity,
        (exerciseIntensity: ExerciseIntensity) => exerciseIntensity.HEALTH_DOCUMENTS,
