...
export class HealthDocumentService {
  ...
  constructor(private readonly _healthDocumentRepository: HealthDocumentRepository) { }

  async createHealthDocument(user_id: number, healthDocument: CreateHealthDocumentDto) {
    ...
    if (healthDocument.EXERCISE_INTENSITY_ID) {
      healthDocumentEntity.EXERCISE_INTENSITY = { ID: healthDocument.EXERCISE_INTENSITY_ID } as any;
    }

    return this._healthDocumentRepository.create(healthDocumentEntity);
  }

  async updateHealthDocument(id: number, healthDocument: UpdateHealthDocumentDto) {
